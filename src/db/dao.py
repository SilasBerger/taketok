...
class Dao:
    ...
    def _set_download_status(self, source_url: str, status: str):
        self._db.write("update video set download_status = ? where source_url = ?", (status, source_url))
    ...
    def save_author_if_not_exists(self, author_id):
        ...
        return self._db.write("insert into author (author_id) values (?)", (author_id,))
    ...
    def _insert_hashtag_if_not_exists(self, hashtag):
        ...
        if result is not None:
            return result[0]
        return self._db.write("insert into hashtag (hashtag) values (?)", (hashtag,))
    ...
    def _insert_video_hashtag_associations(self, video_rowid, hashtag_rowids):
        ...
        for hashtag_rowid in hashtag_rowids_to_associate:
            ...
            self._db.write(insert_statement, (video_rowid, hashtag_rowid))
    ...
    def _insert_challenge_if_not_exists(self, challenge):
        ...
        return self._db.write("insert into challenge (id, title, description) values (?, ?, ?)", (
            challenge["id"],
            challenge["title"],
    ...
    def insert_transcript(self, video_rowid, transcript):
        self._db.write("update video set transcript = ? where ROWID = ?", (transcript, video_rowid))
